...
var ensureTrain = function ensureTrainner(req,res,next){
    if(req.isAuthenticated()){
        next();
    }
    else{
        req.flash("info", "you must be a trainner to access this page")
        res.redirect("/login")
    }
}
...
module.exports = {ensureAuthenticated: ensureAuth, 
    ensureTrainner: ensureTrain, ensureAdmin: this.ensureAdmin
}